...
export const {
    ...
    addNewToken: async (userID, token) => {
        try {
            ...
            // start add
            await Auth.findOneAndUpdate({ user: userID }, { $push: { listTokens: { value: token } } });
    ...
    lockAllToken: async (userID) => {
        try {
            await Auth.findOneAndUpdate({ user: userID }, { $set: { 'listTokens.$[].status': 'Locked' } });
